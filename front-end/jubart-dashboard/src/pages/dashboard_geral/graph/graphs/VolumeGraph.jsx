...
function VolumeGraph() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            exportData.forEach(item => {
                const exportValue = parseFloat(item.metricKG) / 1000; // Convertendo KG para Toneladas
                yearMap[item.year] = yearMap[item.year] || { year: item.year, export: 0, import: 0 };
                yearMap[item.year].export += exportValue;
            });

            // Processar importações
            importData.forEach(item => {
                const importValue = parseFloat(item.metricKG) / 1000; // Convertendo KG para Toneladas
                yearMap[item.year] = yearMap[item.year] || { year: item.year, export: 0, import: 0 };
            ...
            setChartData({
                ...
                datasets: [
                    {
                        label: 'Exportação (Toneladas)',
                        data: labels.map(year => {
                            const yearData = processedData.find(data => data.year === year);
                            return yearData ? yearData.export : 0;
                        }),
                        borderColor: '#8884d8',
                        backgroundColor: 'rgba(136, 132, 216, 0.5)',
                    },
                    {
                        label: 'Importação (Toneladas)',
                        data: labels.map(year => {
                            const yearData = processedData.find(data => data.year === year);
                            return yearData ? yearData.import : 0;
                        }),
                        borderColor: '#82ca9d',
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Volumes Anuais de Comércio Exterior',
                align: 'center',
                color: '#F0B900', 
                font: {
                    size: 16
